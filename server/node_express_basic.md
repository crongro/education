...
24. 클라이언트가 보낸데이터는 http://localhost:3000/send_email?email=niganiga%40naver.com  이렇게 노출되고, 이것을 서버에서 받기 위해서는 req.param 메서드를 사용해서 얻기.
```javascript
app.get('/send_email', function(req,res,next) {
...
27. 그런데 POST데이터를 받으려면 req.param으로 처리가 안되고, bodyParser 라는 별도 패키지 설치가 필요.
```shell
